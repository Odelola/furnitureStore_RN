...
const ProductDetailsScreen = () => {
  ...
  const ProductColorSwitcher = () => {
    return (
      <AppBox className="absolute w-16 bg-white shadow-2xl -left-8 z-50 justify-evenly rounded-full items-center" style={{ height: HEIGHTOFPRODUCT / 2, transform: [{ translateY: (HEIGHTOFPRODUCT / 3) }] }}>
  ...
  const ScrollIndicators = () => {
    ...
    return (
      <AppBox className="absolute bottom-8 right-8 z-10 justify-between rounded-full flex-row" style={scrollIndicatorsStyle}>
  ...
  const SectorCutOutFromSquareAsBorderRadiusDebugMethod = () => {
    return (
      <AppBox className="absolute bottom-0 left-0 z-40 bg-white">
        <AppBox className="bg-transparent" style={{ width: 40, height: 40, borderTopRightRadius: 40, borderBottomRightRadius: 400, transform: [{ rotate: '90deg' }] }}>
        </AppBox>
      </AppBox>
    )
  }
  ...
  const AddToFavouritesOrCartButtons = () => (
    <AppBox className="self-center flex-row w-[90%] absolute justify-between items-center z-40 pt-12" style={{ bottom: bottomTabHeight / 2 }}>
    ...
    <AppBox className="flex-1 relative bg-white">
      ...
      <ScrollView className="flex-1" alwaysBounceVertical={false} bounces={false} overScrollMode='never' showsVerticalScrollIndicator={false}>
        <AppBox style={{ paddingBottom: (bottomTabHeight * 2) }}>
          ...
          <AppBox className="relative" style={{ marginLeft: (width - WIDTHOFPRODUCT), width: WIDTHOFPRODUCT, height: HEIGHTOFPRODUCT, borderTopLeftRadius: 50, }}>
          ...
          <AppBox className="mt-8" style={{ paddingHorizontal: MEASUREMENTS.paddingHorizontal }}>
            <AppBox className="flex-row justify-between items-end mb-4 w-full">
              <AppBox>
              ...
              <AppBox className="flex-row items-center gap-x-4">
            ...
            <AppBox className="flex-row gap-x-6 mb-2">
              <AppBox className="flex-row items-center gap-x-2">
                <ReviewStarIcon />
